...
TINFO:2,9,0,"2:15:30"`;
    ...
    it('should handle stderr errors', async () => {
      ...
      await expect(DiscService.getAvailableDiscs()).rejects.toBe('Error occurred');
    });
...
MSG:1005,0,1,"MakeMKV v1.16.5 win(x64-release) started"`;
    ...
    it('should handle empty drive data', () => {
      expect(() => DiscService.parseDriveInfo('')).toThrow('No drive data received from MakeMKV');
    });

    it('should handle malformed drive data', () => {
      const data = 'Invalid data format';
      expect(() => DiscService.parseDriveInfo(data)).toThrow('Invalid MakeMKV drive output format');
    });
  });
...
MSG:1005,0,1,"MakeMKV v1.16.5 win(x64-release) started"`, '');
    ...
    it('should handle stderr errors', async () => {
      ...
      await expect(DiscService.getDiscFileInfo(driveInfo)).rejects.toBe('File info error');
    });
...
TINFO:3,9,0,"3:15:11"`;
    ...
    it('should handle empty file data', () => {
      expect(() => DiscService.getFileNumber('')).toThrow('No data received from MakeMKV');
    });

    it('should handle malformed file data', () => {
      const data = 'Invalid format';
      expect(() => DiscService.getFileNumber(data)).toThrow('Invalid MakeMKV output format');
    });
